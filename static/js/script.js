...
let observerAl = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
...
var com = () => {
  ...
  conOne[2].style.width = "80%";
};

let observeCom = new IntersectionObserver(
  (entries) => {
...
let observerBl = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
...
let rotateImg = () => {
  for (let i = 0; i < 2; i++) {
    let thirdCardImg = document
      .querySelectorAll(".img_info")
...
let observerCl = new IntersectionObserver(
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        for (let i = 0; i < 2; i++) {
...
let contactTriggerButton = document.getElementsByClassName(
  "contact_trigger_button"
)[0];
...
window.addEventListener("scroll", () => {
  if (
    window.scrollY > 0 &&
    appbox_a.classList.contains("appbox_a_display") == true
  ) {
    appbox_a.classList.remove("appbox_a_display");
  }
  if (
    window.scrollY > 0 &&
    contact.classList.contains("contact_trigger") == true
  ) {
    contact.classList.remove("contact_trigger");
  }
  if (
    window.scrollY > 0 &&
    appbox_dots[0].classList.contains("appbox_dots_trigger") == true
  ) {
    for (let i = 0; i < appbox_dots.length; i++) {
      appbox_dots[i].classList.toggle("appbox_dots_trigger");
    }
